...
export const getStudents = asyncHandler(async (req, res) => {
  ...
  let query = 'SELECT id, name, email, student_id, department, year, phone, bio, cgpa, status, role, created_at FROM users WHERE role = ?';
...
export const getStudent = asyncHandler(async (req, res) => {
  ...
  const [students] = await pool.query(
    'SELECT id, name, email, student_id, department, year, phone, bio, cgpa, date_of_birth, profile_image, status, created_at FROM users WHERE id = ?',
    [req.params.id]
  );
  ...
  const [eventStats] = await pool.query(
    'SELECT COUNT(*) as total FROM event_registrations WHERE student_id = ?',
    [req.params.id]
  );
  student.stats = {
...
export const updateStudent = asyncHandler(async (req, res) => {
  ...
  const { name, phone, department, year, bio, cgpa, dateOfBirth } = req.body;
  const [students] = await pool.query('SELECT * FROM users WHERE id = ?', [req.params.id]);
  if (students.length === 0) {
    return res.status(404).json({
      success: false,
      message: 'Student not found'
  ...
  await pool.query(
    'UPDATE users SET name = ?, phone = ?, department = ?, year = ?, bio = ?, cgpa = ?, date_of_birth = ? WHERE id = ?',
    [name, phone || null, department || null, year || null, bio || null, cgpa || null, dateOfBirth || null, req.params.id]
  );
  const [updatedStudents] = await pool.query(
    'SELECT id, name, email, student_id, department, year, phone, bio, cgpa, date_of_birth, profile_image, status FROM users WHERE id = ?',
    [req.params.id]
  );
  res.json({
    success: true,
